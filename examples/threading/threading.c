...
void* threadfunc(void* thread_param)
{
    ...
    // wait
    result = usleep(thread_func_args->wait_to_obtain_ms * 1000);  // ms to us
    if ( result != 0 ) {
        ERROR_LOG("sleep failed with %d", result);
    ...
    if ( result != 0 ) {
        ERROR_LOG("pthread_mutex_lock failed with %d", result);
        return thread_func_args;
    }

    // wait 
    result = usleep(thread_func_args->wait_to_release_ms * 1000);  // ms to us
    if ( result != 0 ) {
        ERROR_LOG("sleep failed with %d", result);
        return thread_func_args;
    }
    ...
    if ( result != 0 ) {
        ERROR_LOG("pthread_mutex_unlock failed with %d", result);
        return thread_func_args;
    }
...
bool start_thread_obtaining_mutex(pthread_t *thread, pthread_mutex_t *mutex,int wait_to_obtain_ms, int wait_to_release_ms)
{
    ...
    */
    ...
    if(result != 0){
        printf("Failed to initialize thread, error was %d",result);
        return false;
    }
